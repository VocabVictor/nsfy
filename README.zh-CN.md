...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/性能-完整基准已验证-lightgrey" />
</p>
...
│  已验证 10k 连接 · 完整数据见 PERFORMANCE.md   │      │
└──────────────────────┬─────────────────────────┘      │
                       │ wss://<topic>/ws               │
         ┌─────────────┼─────────────┐                  │
         ▼             ▼             ▼                  │
...
下面的 HTTP 地址仅限同机回环测试。局域网、公网和其他非回环地址必须使用
`https://`，实时订阅必须使用 `wss://`；服务端和所有客户端都会拒绝远程明文连接。
...
```bash
# WebSocket — 实时只读订阅；发布统一使用 HTTPS POST
...
优先级 1（最低）到 5（最高）。tags 是字符串数组。category 是最多 8 层的
分类路径；不传时与旧消息完全兼容。桌面端和 Android 可以按父分类及其子分类筛选。

桌面端和 Android 都支持不打开 GUI 的命令行操作，参见
[CLI 使用说明](CLI.zh-CN.md)。
...
| 参数 / 环境变量 | 默认值 | 作用 |
|---|---|---|
| `--auth-token` / `NSFY_AUTH_TOKEN` | 本机可不设 | 使用 `Authorization: Bearer <token>` 鉴权；不再接受 URL 查询参数，避免 Token 进入访问日志。非回环监听必须设置。 |
...
| `--cache-size` / `NSFY_CACHE_SIZE` | 100 | 每个 topic 在内存中保留的最近消息数，用于轮询与流订阅重放。 |
| `--stream-buffer-size` / `NSFY_STREAM_BUFFER_SIZE` | 256 | WS/SSE 客户端短暂变慢时可积压的实时消息数。突发流量特别大时可以调高，但会增加每个活跃 topic 的内存。 |
...
不设置 `--auth-token` 只适用于回环地址开发。只要允许其他设备连接，服务端就会要求 Token，并且只接受 Authorization 请求头。

单个请求处理过程中的 panic 会被捕获并转成 `500`，不会把整个进程（以及其他所有客户端的连接）一起拖垮。

## 持久化

nsfyd 强制使用 SQLite。默认数据库是当前目录的 `nsfy.db`，生产环境应明确指定路径：

```bash
./target/release/nsfyd --db-path /var/lib/nsfy/nsfy.db
```

SQLite 使用 WAL 和 `synchronous=FULL`。并发排队的发布会合并进有上限的批量事务，但每个请求仍然要等所属事务真正提交后才广播并返回成功；数据库打开、迁移或加载失败时服务端直接停止，不会退回不可靠的纯内存模式。

| 参数 / 环境变量 | 默认值 | 作用 |
|---|---|---|
| `--db-path` / `NSFY_DB_PATH` | `nsfy.db` | 必需的 SQLite 数据库文件路径。 |
...
启动时，nsfyd 会在开始接受连接之前，把每个 topic 最近的消息重新加载回内存。

### TLS

nsfyd 支持 `--tls-cert` 与 `--tls-key`，直接提供 HTTPS/WSS。非回环监听始终拒绝明文，无法通过参数绕过。使用同机 TLS 反向代理时，让 nsfyd 只监听回环地址；回环 HTTP 仅用于本机开发和同机代理回源。
...
| 端 | 技术 | 体积 | 说明 |
|----|------|------|------|
| 服务端 | Rust + axum | 依平台而定 | 单二进制，systemd 一行配，内置 SQLite 持久化 |
